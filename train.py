...
if config["logger"] == "wandb" and config["run_type"] == "train":
    # init wandb
    wandb.init(
        project="GlobalPointer_" + config["exp_name"],
    ...
    model_state_dict_dir = os.path.join(
...
def data_generator(data_type="train"):
    ...
    if data_type == "train":
        train_data_path = os.path.join(
            config["data_home"], config["exp_name"], config["train_data"]
        )
        train_data = load_data(train_data_path, "train")
        valid_data_path = os.path.join(
            config["data_home"], config["exp_name"], config["valid_data"]
        )
        ...
        valid_data_path = os.path.join(
            config["data_home"], config["exp_name"], config["test_data"]
        )
        valid_data = load_data(valid_data_path, "valid")
        train_data = []
                ...
                if valid_f1 > config["f1_2_save"]:  # save the best model
                    ...
                    torch.save(
                        ...
                        os.path.join(
                            ...
                            "model_state_dict_{}.pt".format(model_state_num),
                        ),
                    )
